...
export function LoginForm() {
  ...
  useEffect(() => {
    if (state.success && !isExiting) {
      setIsExiting(true);
      setTimeout(() => router.push("/adpanel/products"), 480);
    }
  }, [state.success, isExiting, router]);
    ...
    >
      <form action={formAction} className="mt-8 space-y-3">
        ...
        >
          ...
          <span>{isPending ? "Проверяем..." : isExiting ? "Входим..." : "Войти"}</span>
        </button>
      </form>
    </motion.div>
  );
